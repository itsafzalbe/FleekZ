...
from .models import Video, VideoLikes
...
def video_detail(request, video_id):
    ...
    if request.user.is_authenticated:
        like = VideoLikes.objects.filter(user=request.user, video=video).first()
        if like:
...
def video_vote(request, video_id):
    ...
    value = VideoLikes.LIKE if vote_type == "like" else VideoLikes.DISLIKE
    existing_vote = VideoLikes.objects.filter(user=request.user, video=video).first()

    if existing_vote:
        if existing_vote.value == value:
            if value == VideoLikes.LIKE:
                video.likes -= 1
            else:
                video.dislikes -= 1
        ...
        VideoLikes.objects.create(user=request.user, video=video, value=value)
        if value == VideoLikes.LIKE:
            video.likes +=1
        else:
            video.dislikes += 1
